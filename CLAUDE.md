...
- **UFC v1 (added 2026-07-12) — everything here is provisional, and the data source itself is unverified, not just the constants.** `fetchers/ufc.py` scrapes ufcstats.com (no JSON API exists, official or hidden — unlike every other sport in this repo) using CSS class names inferred from the site's long-stable, widely-scraped public structure; this repo's build/test containers cannot reach ufcstats.com at all (confirmed 2026-07-12, same proxy block as espn.com), so those class names have **not been live-verified** — check them against a real response once deployed. `models/ufc_model.py`'s CONFIDENT_RD (60), MODEL_PROB_CAP (0.82), REACH_COEF (0.015), AGE_DECLINE_START (34), and the LEAGUE_AVG_WIN/LOSS_KO/SUB_RATE fallbacks (0.45/0.20) are all guesses, not fitted to real UFC results. Architecturally different from every other sport here on purpose: no score model at all (MMA has no continuous score to predict — win probability comes from Glicko-2 blended with a career-stats logistic, not Poisson/NB), and a second, separate method-of-victory model (KO-TKO/submission/decision) that has no equivalent anywhere else in this codebase. Chose Glicko-2 over plain Elo specifically because fighters go inactive for months/years between fights — Elo has no notion of "we're less sure about this rating now," Glicko-2's RD does (see the composite_win_prob blend-weight logic, which literally trusts Glicko-2 less as RD grows). Known unmodeled gap: true "ring rust" needs days-since-last-fight, which needs a per-event date lookup `fetch_fight_history` doesn't do (the per-fighter fight table has no date column, only an opponent/method/round/event); also, `models/glicko.py`'s RD only grows on a recorded win/loss, not on elapsed real time with zero fights, so a 3-year-layoff fighter won't show extra uncertainty from inactivity alone — a real fix needs a periodic time-decay pass that doesn't exist for *any* sport's Glicko-2 usage in this repo today (not something broken by this addition, just an existing gap this sport would benefit most from fixing). Draws are real in MMA (majority draws, <2%) but deliberately not modeled — `prob_draw` is hardcoded 0.0 in the persisted prediction row rather than a fabricated near-zero estimate. Bet thresholds reuse the same conservative 65%/62pp levels as rugby, for the same zero-resolved-predictions reason. Recalibrate everything once a season of real, resolved UFC predictions exists — and once the ufcstats.com scrape itself is confirmed working.
