...
type: function
file: fetchers/ufc.py
purpose: One-shot diagnostic, built in FROM THE START this time (2026-07-14) rather than added after a failure like it was for ufcstats.com. Live-testing 2026-07-15 found: Tapology dead (Cloudflare 403); FightMatrix's ranking table JS-rendered; ESPN's core API confirms "ufc" (id 3321) is a real registered league, but the site API 404s on /athletes for BOTH the slug and the numeric id — unlike rugby's bug (where the numeric id fixed it), this is a different signal: ESPN may simply not expose per-fighter profiles for MMA via /athletes at all, even though the league is registered. /scoreboard, by contrast, is CONFIRMED working (200, real event) for the slug (numeric id 400s here — "Failed to get events endpoint"). Extended same day again: now also runs fetch_espn_scoreboard_range + _fighter_fights_from_scoreboard + _method_rates for sample_fighter. **Extended a THIRD time same day** after live-testing found the wide-range call returns 0 events while the no-params call finds 1: espn_scoreboard_probe_slug/numeric_id now capture each returned event's actual name/date/status/competitor names (not just a count) instead of guessing what that "1 event" even is; a new espn_scoreboard_chunk_probe tests five distinct 90-day windows going back a year (this week, ~3/6/9/12 months back) and reports each window's raw event count + top-level JSON keys + a raw body snippet when empty, so the actual shape of ESPN's historical MMA coverage (or lack of it) is visible directly instead of inferred from one aggregate number.
inputs: sample_fighter: str = "Jon Jones"
outputs: dict (espn_athletes_status/espn_league_discovery/espn_candidate_slug_probe/espn_ufc_league_entry/espn_numeric_id_probe/espn_scoreboard_probe_slug/espn_scoreboard_probe_numeric_id/espn_scoreboard_chunk_probe/espn_parsed_athlete_sample/espn_parsed_bio/espn_scoreboard_range_event_count/espn_scoreboard_range_sample/espn_sample_fighter_fights/espn_sample_fighter_method_rates, plus the existing FightMatrix/Tapology raw status codes, content lengths, profile-link booleans, raw snippets, and table/anchor/API-hint diagnostics)
