...
type: function
file: models/nrfi_model.py
purpose: Trains XGBoost on data/nrfi_dataset.csv. Dynamic train/val/test split: train=all seasons except two most recent, val=second-most-recent, test=most-recent. With 2022-2024: train=2022, val=2023, test=2024. With 2022-2026: train=2022-2024, val=2025, test=2026. xwoba_against removed from FEATURES (statcast_pitcher_exitvelo_barrels does not return xwOBA; barrel_pct/hard_hit_pct provide the same signal). Prints feature coverage diagnostic, Brier score, AUC-ROC, accuracy at 50% and 65% thresholds, top feature importances, and calibration curve (8-bin quantile, predicted% vs actual%). Platt scaling (LogisticRegression) only applied when val AUC > 0.52 — skipped otherwise to prevent noise amplification causing inverted calibration curve. Quality filter uses home_fip/home_k_pct/away_fip/away_k_pct. Saves models/nrfi_xgb.json and models/nrfi_calibrator.pkl. Run via: python models/nrfi_model.py --train
