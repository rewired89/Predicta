...
type: function
file: fetchers/openinsider.py
purpose: get_latest_filings(limit, days) is a MARKET-WIDE feed (openinsider.com's "latest-insider-trading" page, not the per-symbol screener the rest of this file uses) — every recent Form 4 open-market Buy/Sell filing across all companies, newest first. Column layout confirmed live 2026-09-07 (17 cells per real data row, not the originally-assumed 13 — see _parse_latest_filings_html's docstring): Filing Date/Trade Date/Ticker/Company/Insider Name/Title/Trade Type/Price/Qty/Owned/ΔOwn/Value plus 4 unused trailing empty cells. Only keeps exact "P - Purchase"/"S - Sale" rows (excludes "S - Sale+OE" etc., same convention as get_recent_insider_activity). diagnose_latest_filings() is the live /copy-trades-diag check. Fails safe to [] on any fetch/parse error. **ROOT CAUSE FOUND AND FIXED 2026-09-07**, from the user pasting a real /copy-trades-diag capture (raw_best_row_cells/raw_best_row_html_snippet) showing fetch_ok=true, a real 125KB page, 100 real 17-cell data rows (raw_cell_count_histogram), yet parsed_row_count=0. The 13-vs-17 cell-count assumption itself wasn't the blocker (17 >= 13 still passed that check) — every field except the ticker parsed correctly at its existing index. The actual bug: the ticker cell's `<a>` tag carries an `onmouseover="Tip('<img src=...>', ...)"` chart-tooltip attribute — a literal "<img ...>" as TEXT inside a quoted HTML attribute, not a real nested tag — and `_TAG_RE`'s naive `<[^>]+>` tag-stripper matched up to the FIRST `>` it found, which landed inside that embedded fake img tag, truncating the real wrapper early and leaving garbage text that always failed the ticker.isalnum() check, silently dropping every single row. Fixed by extracting the ticker from the anchor's `href="/TICKER"` directly via a new `_TICKER_HREF_RE` regex instead of relying on the corrupted inner text — verified against the user's exact real row plus a synthetic Purchase-side row and a non-data ad row. A second, smaller bug found and fixed in the same pass: `_parse_number()` lost the sign on values like "-$1,600,917" — the minus sign sits before the "$", so _NUM_RE's leading `[-+]?` never reached the digits and matched "1600917" as positive; fixed by stripping "$"/"%" (not just ",") before the numeric regex search.
