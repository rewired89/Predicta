...
type: function
file: fetchers/ufc.py
purpose: Scrapes a Sherdog fighter profile for record + bio via best-effort TEXT PATTERN regexes (win-loss-draw digit triple, HEIGHT/REACH/AGE keyword-adjacent numbers) rather than assumed CSS classes, since the exact markup hasn't been live-verified from this sandbox — same resilience approach as fetch_tapology_profile. Individual fields are None when their pattern isn't found. Verified against synthetic Sherdog-shaped HTML before push (parsed a 28-1-0 record, height, reach, age correctly). Returns {} on failure.
inputs: url: str
outputs: dict {name, wins, losses, draws, height_in, reach_in, age}
calls: _get, BeautifulSoup, re.search
called_by: enrich_ufc_fighters, diagnose
...
type: function
file: fetchers/ufc.py
purpose: Extracts a fighter's fight history from their Sherdog profile page. Tries a table whose header row mentions "method" or "opponent" (flexible header matching, not an exact class name) first; falls back to the same "Win/Loss ... Method" text-pattern scan fetch_tapology_fight_history already uses if no such table is found. Verified against synthetic HTML before push. Returns [{result, opponent, method}]. Empty list on failure.
inputs: url: str, limit: int = 15
...
---
name: _wikipedia_search
type: function
file: fetchers/ufc.py
purpose: Finds the best-matching Wikipedia page title for a fighter name via Wikipedia's official search API (action=query&list=search). Returns None on failure or no results.
inputs: name: str
outputs: Optional[str]
calls: httpx.Client.get
called_by: fetch_wikipedia_mma_record, diagnose
mutates: none
---

---
name: fetch_wikipedia_mma_record
type: function
file: fetchers/ufc.py
purpose: Fetches a fighter's MMA fight history from their Wikipedia page via the Action API (action=parse&prop=text). Used specifically because Wikipedia can't hit the Cloudflare/JS-challenge walls that killed ufcstats.com/Tapology or the JS-rendering issue that killed FightMatrix's ranking table. Finds the "Mixed martial arts record" heading, then the following wikitable, and parses rows via FLEXIBLE HEADER-NAME matching (looks for "res"/"opponent"/"method"/"event"/"date" substrings in the header row rather than assuming a fixed column order) since the exact table hasn't been live-verified from this sandbox. Verified against synthetic wikitext-rendered HTML before push. Returns [] on failure or if no record table is found (e.g. fighter has no Wikipedia page).
inputs: name: str
outputs: list[dict] {result: 'W'|'L'|'D'|'NC', opponent, method, event, date}
calls: _wikipedia_search, httpx.Client.get, BeautifulSoup
called_by: enrich_ufc_fighters, diagnose
mutates: none
---
...
type: function
file: fetchers/ufc.py
purpose: Main entry point. REWRITTEN 2026-07-15 (second rewrite same day) — per direct user request, ESPN is dropped from the active pipeline entirely (its functions stay in the file, just unused here) after its /scoreboard historical-range behavior stayed unconfirmed through repeated live tests. New chain: Sherdog (search_sherdog_fighter → fetch_sherdog_profile for bio, fetch_sherdog_fight_history for record/method) is PRIMARY; Wikipedia (fetch_wikipedia_mma_record) is the FALLBACK when Sherdog resolves nothing or returns no fight history; FightMatrix ranking stays layered in as a supplementary signal as before. Data-leakage guard: excludes any fight against the OTHER fighter in this matchup (`opponent_of` + `_name_matches`) from each fighter's own history, since neither Sherdog nor Wikipedia support a date-filtered query the way ESPN's /scoreboard did — matching by opponent name is actually more precise than a date cutoff for the specific "these two already fought" case. `before_date` param kept for call-site compatibility (analyze_ufc.py still passes match_date) but is otherwise unused now. A fighter's dict is {} only when Sherdog, Wikipedia, AND FightMatrix all fail to resolve them.
inputs: name_a: str, name_b: str, before_date: Optional[str] = None
outputs: dict {a: dict, b: dict} — each with wins/losses/draws/height_in/reach_in/age/name (from Sherdog when resolved) plus win_ko_rate/win_sub_rate/loss_ko_rate/loss_sub_rate/fight_history_count (from whichever of Sherdog/Wikipedia supplied fight history) plus fm_rank/fm_rating (from FightMatrix) when resolved
calls: search_sherdog_fighter, fetch_sherdog_profile, fetch_sherdog_fight_history, fetch_wikipedia_mma_record, _name_matches, _method_rates, lookup_fightmatrix_fighter
...
type: function
file: analyze_ufc.py
purpose: Full UFC pipeline entry point — parse query (ai_agent_ufc) → Sherdog record/bio/fight-history with a Wikipedia fallback + FightMatrix ranking (fetchers/ufc, REWRITTEN 2026-07-15 second time same day: ESPN dropped from the active pipeline after its /scoreboard historical-range behavior stayed unconfirmed through repeated live tests — see fetchers/ufc.py:enrich_ufc_fighters; leakage guard is now opponent-name-based instead of date-based, before_date kept only for call-site compatibility) → Glicko-2 rating (models/glicko.py, sport="ufc", surface=weight_class) → stats-logistic blend (models/ufc_model, ranking+reach+age based — reach/age are available again when Sherdog's bio parse succeeds, unlike the ESPN-only period) → MODEL_PROB_CAP safety clamp → method-of-victory model for both possible winners (fed by real Sherdog/Wikipedia fight-history finish rates) → markets (method of victory, fight-finishes%, goes-the-distance%) → bet recommendations + Kelly stake → persist match/signals (fm_rank/win_ko_rate/win_sub_rate/fight_history_count/reach_in/age per fighter)/prediction (sport='ufc', prob_draw hardcoded 0.0 — draws are rare and explicitly NOT modeled) → narrate. Returns {"status": "insufficient_data", ...} without persisting when neither source has data for either fighter.
inputs: user_query: str, bankroll: float = 1000.0
outputs: dict (see analyze_rugby.run_rugby_analysis's return shape for the parallel fields; UFC-specific additions are markets.method_of_victory/fight_finishes_pct/goes_the_distance_pct, and there's no margin/totals market since there's no score; data_sources is now ["sherdog", "wikipedia", "fightmatrix", "glicko2"])
