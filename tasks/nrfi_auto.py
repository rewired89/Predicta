...
def _push_day(target_date: str, run_type: str = "run") -> list[str]:
    ...
    report_md = rp.read_text() if rp.exists() else ""
    if report_md and _push_file(
        f"data/nrfi_reports/{target_date}.md", report_md,
        f"nrfi(railway): report {target_date}"):
        pushed.append("report")

    # Stable "latest" summary at a FIXED path — overwritten every run so the
    # newest scan is always one known file (data/nrfi_latest.md), no hunting for
    # today's date. This is the file to read to check the most recent results.
    header = (
        f"> **Latest NRFI scan** — `{run_type}` at "
        f"{datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')} for {target_date}.\n"
        f"> Auto-generated by the Railway scheduler; overwritten every run. "
        f"Always holds the newest results.\n\n"
    )
    if _push_file("data/nrfi_latest.md", header + report_md,
                  f"nrfi(railway): latest [{run_type} {target_date}]"):
        pushed.append("latest")
